...
class cfg():
    ...
    def get_args(self):
        ...
        parser.add_argument("--model_name", default="NewEA", type=str, choices=["EVA", "MCLEA", "MSNEA", "NewEA"], help="model name")
        ...
        # --------- NewEA -----------
        parser.add_argument("--hidden_size", type=int, default=100, help="the hidden size of NewEA")
        parser.add_argument("--intermediate_size", type=int, default=400, help="the hidden size of NewEA")
        parser.add_argument("--num_attention_heads", type=int, default=5, help="the number of attention_heads of NewEA")
        parser.add_argument("--num_hidden_layers", type=int, default=2, help="the number of hidden_layers of NewEA")
        ...
        parser.add_argument("--mask_path", default=None, type=str, nargs='?', help="Optional path to mask JSON file")
        parser.add_argument("--ent2id_path", default="ent2id", type=str, help="entity 2 id")
    ...
    def update_train_configs(self):
        ...
        if self.cfg.mask_path:
            self.cfg.mask_path = osp.join(self.cfg.data_path, self.cfg.data_choice, self.cfg.data_split, self.cfg.mask_path)
        print(self.cfg.mask_path)
        self.cfg.ent2id_path = osp.join(self.cfg.data_path, self.cfg.data_choice, self.cfg.data_split, self.cfg.ent2id_path)

        print(self.cfg.ent2id_path)
        ...
        if self.cfg.enable_sota:
            if self.cfg.il:
                ...
                if self.cfg.data_choice == "DBP15K" or "FBYG15K" or "FBDB15K" in self.cfg.data_choice:
                    self.cfg.epoch = 250
                else:
                    self.cfg.epoch = 500
