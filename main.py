...
class Runner:
    ...
    def model_choise(self):
        assert self.args.model_name in ["EVA", "MCLEA", "MSNEA", "NewEA"]
        if self.args.model_name == "NewEA":
            self.model = NewEA(self.KGs, self.args)
    ...
    def il_for_ea(self):
        with torch.no_grad():
            if self.args.model_name in ["NewEA"]:
