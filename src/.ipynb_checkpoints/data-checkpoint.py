...
def load_eva_data(logger, args):
    ...
    att1 = os.path.join(file_dir, 'attribute1.txt')
    att2 = os.path.join(file_dir, 'attribute2.txt')
    a1 = os.path.join(file_dir, 'att_sum_left_noname.pkl')
    a2 = os.path.join(file_dir, 'att_sum_right_noname.pkl')
    # att_features = load_attr([a1, a2], ENT_NUM, ent2id_dict, 1000)  # attr
    att_features, mask_a = load_attr([a1, a2], [att1, att2], ENT_NUM)  # attr
...
def load_attr(pkl_files, txt_files, total_entity_num):
    """
    加载两个属性嵌入.pkl文件（左右KG），补齐所有实体向量为 [total_entity_num, dim]。
    若某实体无向量，则补 0。
    返回值为 numpy ndarray。
    """
    assert len(pkl_files) == 2, "需要两个pkl文件，分别是左右KG的属性向量"

    # 加载两个dict
    with open(pkl_files[0], 'rb') as f1:
        left_attr = pickle.load(f1)
    with open(pkl_files[1], 'rb') as f2:
        right_attr = pickle.load(f2)

    # 获取维度
    sample_vec = next(iter(left_attr.values()))
    emb_dim = len(sample_vec)

    attr_feat = np.zeros((total_entity_num, emb_dim), dtype=np.float32)

    for ent_id, vec in left_attr.items():
        ent_id = int(ent_id)
        if ent_id < total_entity_num:
            attr_feat[ent_id] = np.array(vec, dtype=np.float32)

    for ent_id, vec in right_attr.items():
        ent_id = int(ent_id)
        if ent_id < total_entity_num:
            attr_feat[ent_id] = np.array(vec, dtype=np.float32)

    mask = np.zeros(total_entity_num, dtype=np.uint8)
